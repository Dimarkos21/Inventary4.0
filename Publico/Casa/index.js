...
(async () => {
  try {
     ...
     const tabla = document.querySelector(".chart");
          ...
          const clientesD = data.filter(obj => obj.paciente).length
          const productosD = productos.filter(obj => obj.nombre).length;
          const numerosD = data.filter(obj => obj.telefono).length;
          const costoD = productos.filter(obj => obj.costo);

          const precioD = productos.filter(obj => obj.precio);
        ...
        function calcularSumaPrecios(data) {
          // Filtrar los objetos que tienen la propiedad 'precio'
          const objetosConPrecio = data.filter(obj => obj.precio);
      
          // Mapear los valores de 'precio' y convertirlos a números
          const valoresPrecio = objetosConPrecio.map(obj => Number(obj.precio));
      
          // Sumar todos los valores de 'precio' con reduce
          const sumaTotal = valoresPrecio.reduce((acumulador, valor) => acumulador + valor, 0);
        ...
        const t = calcularSumaPrecios(precioD); 
        const G = t - c; 
        
        console.log(`Costo total: ${c}`);
console.log(`Precio total: ${t}`);
console.log(`Ganancia calculada: ${G}`);
